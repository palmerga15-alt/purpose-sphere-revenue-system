// ============================================================
// FINANCIAL INTELLIGENCE — SECURE API CLIENT (Phase 2)
// window.FinanceApiClient
//
// Foundation for fetching real financial data from a private,
// authenticated backend. This repository ships NO backend, NO
// endpoint, and NO credentials:
//
//   - The endpoint comes only from window.FINANCE_CONFIG, which
...
//   - Financial payloads and tokens are NEVER logged. Errors
//     surface as short state codes only.
//   - Every response is validated against the schemaVersion-1
//     contract (FinanceValidate) before it may render; anything
...
// ============================================================
window.FinanceApiClient = (function () {
  "use strict";
  ...
  async function fetchData() {
    ...
    const v = window.FinanceValidate.validateData(json);
    if (!v.ok) {
      // Payload is rejected and discarded. Log a count only — never the payload.
      console.warn("[finance] secure response rejected — " + v.errors.length + " validation issue(s)");
